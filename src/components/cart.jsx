...
export default class Cart extends React.Component{
    ...
    placeOrder = () => {
        if(!this.state.address) {
            ...
            placeOrder(this.state.cartID,this.state.address).then(res => {
                this.cartStepperChange()
            })
        } 
    }
            ...
            <div className="cart_container">
                ...
                <div className="shopping_cart">
                    ...
                    <div className='shopping_cart_service'>
                        <div>
                            ${this.state.cartData.price} per month 
                        </div>
                        <div>
                            {this.state.cartData.name}
                        </div>
                    </div>
                    </div>
                    <div>
                        <div className='shopping_cart_buleFont'>
                        basic Pack Details
                        </div>
                    ...
                    <div className='service_card_subtotal_media'>
                        <div className='service_card_subtotal'>
                            <div>
                            Subtotal(1 item) : ${this.state.cartData.price}
                            </div>
                            {this.state.isOrderPlaced?
                                <div className='orderPlaced'>Order Placed</div>
                            :
                                <div>
                                    <div className={this.state.addressBarView?'show':'hide'}>
                                        <Button variant='contained' color='primary' onClick={this.placeOrder}> Place Your Order</Button>
                                    </div>
                                    <div className={!this.state.addressBarView?'show':'hide'}>
                                        <Button variant='contained' color='secondary' onClick={this.checkOutOrder}> Process to checkout</Button>
                                    </div>
                                </div>
                            }
                ...
                <div className='shopping_cart_buleFont'>Sub Total ( 1 item ): ${this.state.cartData.price}</div>
                <div className={this.state.addressBarView?'shopping_address show':'hide'}>
                    <div>
                        <TextareaAutosize aria-label="minimum height" value={this.state.address} onChange={e => this.setState({address:e.target.value})} rowsMin={3} placeholder="Enter Your Address" />
                    </div>
                    <div>
                        <p>payment method</p>
